...
init ports, whereas same-bank traffic is capped at ~1 access/cycle no matter
...
async def mem_ss_throughput(dut):
    ...
    # A single pipelined port sustains ~1 access/cycle to its own bank.
    assert d1 > 0.8, f"single-port disjoint throughput too low: {d1:.3f}"
    # Disjoint banks: throughput scales with active ports (multi-bank concurrency).
    assert d4 > 2.5, f"4-port disjoint throughput did not scale: {d4:.3f}"
    assert d4 > 1.8 * d1, (
    ...
    # Same bank is the serialization floor: one slice caps at ~1 access/cycle.
    assert s4 < 1.3, f"same-bank throughput exceeded one slice's ceiling: {s4:.3f}"
    assert d4 > 1.8 * s4, (
        f"banked concurrency ({d4:.3f}) did not beat single-bank serialization ({s4:.3f})")
    # Writes are as concurrent as reads.
    assert wr4 > 2.5, f"4-port disjoint write throughput did not scale: {wr4:.3f}"
