...
def parse_soc_top(path: Path) -> tuple[dict[str, int], list[tuple[int, str, str]]]:
    ...
    # The RTL widens the 32-bit window parameters to the xbar rule fields
    # with a zero-extension concatenation; flatten it so the rule regex sees
    # the bare parameter names.
    text = re.sub(r"\{\s*\d+'h0\s*,\s*(\w+)\s*\}", r"\1", text)
    values: dict[str, int] = {}
