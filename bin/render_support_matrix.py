...
OUT_PATH = REPO_ROOT / "docs" / "source" / "support_matrix.rst"
...
def heading(text: str, char: str) -> list[str]:
    return [text, char * len(text)]


def table(headers: list[str], rows: list[list[str]]) -> str:
    """Render a list-table; cell widths vary too much for a grid table."""
...
def render_board_matrix(board: dict[str, str | list[str]], cores: dict[str, dict[str, str | list[str]]]) -> str:
    ...
    lines = [
        *heading(f"{board['display']} (``{board_name}``)", "-"),
        "",
...
def render() -> str:
    ...
    lines = [
        ".. Generated by bin/render_support_matrix.py - do not edit by hand.",
        "",
        *heading("CoreJack Support Matrix", "="),
        "",
    ...
    lines.extend([*heading("Core ISA Summary", "-"), "", render_core_matrix(cores), ""])
    return "\n".join(lines)


def main() -> None:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--check", action="store_true", help="fail if docs/source/support_matrix.rst is stale")
