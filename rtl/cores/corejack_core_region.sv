...
module corejack_core_region #(
  ...
  if (CoreType == platform_pkg::CoreIbex) begin : gen_ibex
  ...
  end else if (CoreType == platform_pkg::CoreCv32e40p) begin : gen_cv32e40p
    ...
    assign debug_unavailable_o = 1'b0;
  end else if (CoreType == platform_pkg::CoreCv32e40x) begin : gen_cv32e40x
    ...
    assign debug_unavailable_o = 1'b0;
  end else if (CoreType == platform_pkg::CoreCv32e40s) begin : gen_cv32e40s
  ...
  end else if (CoreType == platform_pkg::CoreCva6) begin : gen_cva6_requires_axi
...
`endif
  end else if (CoreType == platform_pkg::CoreServ) begin : gen_serv
    ...
    assign alert_major_bus_o      = 1'b0;
    assign debug_unavailable_o    = 1'b1;
  end else if (CoreType == platform_pkg::CorePicorv32) begin : gen_picorv32
    corejack_picorv32_socket_adapter #(
      ...
      .data_rdata_i   (data_rdata_i),
      .data_err_i     (data_err_i),
      // A trapped PicoRV32 is halted forever (no debug module to recover it);
      // report it on the major-internal alert so it is observable.
      .trap_o         (alert_major_internal_o),
      .core_sleep_o   (core_sleep_o)
    );
    ...
    assign alert_major_bus_o      = 1'b0;
    assign debug_unavailable_o    = 1'b1;
  end else if (CoreType == platform_pkg::CoreCvw) begin : gen_cvw
    corejack_cvw_ahb_adapter #(
      .BootAddr (BootAddr + 32'h80)
    ) i_cvw_adapter (
      .clk_i          (clk_i),
      .rst_ni         (rst_ni),
