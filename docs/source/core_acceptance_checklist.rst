...
:doc:`AXI4 fabric <axi4_fabric>`. For the current RV32 cores this means
...
-  ``docs/source/support_matrix.rst`` by running ``make support-matrix`` (this is the
   authoritative per-core/board status table; the top-level README and
   ``docs/source/roadmap.rst`` link to it rather than duplicating it).
-  ``docs/source/roadmap.rst`` if the per-core note needs to change.
-  ``docs/source/core_board_descriptors.rst`` if descriptor semantics changed.
-  ``docs/source/fpga_hardware_smoke.rst`` if board-level hardware expectations changed.
-  ``docs/source/fpga_debug_stepping.rst`` if debug procedure or commands changed.
