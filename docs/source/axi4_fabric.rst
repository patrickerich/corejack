...
`platform rule <#platform-rule-direct-memory-ports-and-the-system-bus>`__.
...
-  the PLIC (``rtl/platform/soc_plic.sv``) through a second ``soc_axi_to_reg``;
   ...
   `Platform Interrupts <#platform-interrupts>`__)
...
already-banked SRAM (see `memory throughput <#memory-throughput-is-no-longer-fabric-limited>`__)
...
.. image:: media/corejack_soc_axi_fabric.svg
   :alt: AXI Fabric
...
.. image:: media/corejack_soc_memory_map.svg
   :alt: Memory Map
...
+-----------+-------------------------------------------------------------------------------------+
| 2         | iDMA transfer completion (sticky flag in the socket adapter; acknowledge with a W1C |
|           | write to DMA window offset ``0xF00`` before completing at the PLIC)                 |
+-----------+-------------------------------------------------------------------------------------+
...
on both. See :doc:`roadmap` for the coupling between ``MemNumBanks``
and the port count.
...
-  revisit ``MemNumBanks`` if a workload shows bank pressure; nine ports now
   ...
   (the bank-count vs port-count coupling; see :doc:`roadmap`)
