...
The current reference descriptor is
:repofile:`cfg/boards/axku5.yaml`.
...
.. code:: yaml
   ...
   debug:
     ...
     # make ... JTAG_ADAPTER=<name>; see docs/source/jtag_wiring.rst for adding a new
     # adapter wrapper.
     jtag_adapter: tigard
...
so it lives once in :repofile:`cfg/validation/uart_banners.yaml`
...
rendered into :doc:`support_matrix` by
``make support-matrix``.
