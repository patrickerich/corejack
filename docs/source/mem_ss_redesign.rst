...
.. _2-clarifications-resolved-with-maintainer:
...
-  **Ingress buffer** (per port, in-order FIFO, depth ``IngressDepth``\ =2). ``gnt = !full``. Only the head is arbitrated -> per-port request order preserved. Depth
...
-  **Per-board bank counts:** the count is now a single constant
   ...
   the FPGA wrapper and the simulation DUT - see :doc:`roadmap` -
