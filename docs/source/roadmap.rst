...
:doc:`support_matrix`. For the validation gates used to
promote a core, see :doc:`core_acceptance_checklist`.
...
-  The ``soc_plic`` platform interrupt controller (standard RISC-V PLIC layout,
   ...
   delivery end to end. See :doc:`axi4_fabric`.
...
:doc:`support_matrix`. Notes per core:
...
-  **CV32E40S** - simulation, FPGA, OpenOCD/GDB load/run with single-step
   ...
   :doc:`core_acceptance_checklist`. Zephyr
   initial.
...
-  Adopt a layered interconnect as the platform endpoint, structured as
   ...
   *Multi-Initiator Architecture (planned)* tab of
   ```media/corejack_soc.drawio`` <media/corejack_soc.drawio>`__ and shown
   below:

.. image:: media/corejack_soc_multi_initiator_architecture_planned.svg
   :alt: Multi-Initiator Architecture (planned)
...
-  Add more cores under the same socket and AXI fabric contract using
   :doc:`core_porting` and the ``make new-core`` scaffold.
-  Add more FPGA boards behind thin board wrappers using
   :doc:`board_porting` and the ``make new-board`` scaffold.
-  Use :doc:`core_acceptance_checklist` as the
...
documented in :doc:`board_porting`. No further board is
scheduled.

.. _system-ip-and-accelerator-expansion:
...
"jack in an IP" pitch in :doc:`about` describes the intent;
...
```media/corejack_soc.drawio`` <media/corejack_soc.drawio>`__, which are
